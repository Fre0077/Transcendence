...
export function googleLoginFunction(clientId: string): Promise<any> {
	// const clientId = env || null;

	if (!clientId) {
		console.error("CLIENT_ID is not defined in environment variables");
		alert("Google login is not configured properly.");
		return Promise.reject(new Error("CLIENT_ID not configured"));
	}
