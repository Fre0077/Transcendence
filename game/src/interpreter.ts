...
export function interpreter(
	...
	bot:Bot | undefined,
	callback :(entry:GameEntry | undefined, player:Player | undefined, bot:Bot | undefined) => void): string | null
{
	...
	switch (msg.method)
	{
		case "JOIN":
			...
			if (jret.status === "success")
			{
				...
				const retBot = retEntry.players.find(p => p.ID === "BOT") ? new Bot() : undefined;
				/* store variables */
				callback(retEntry, retPlayer, retBot);
			}
